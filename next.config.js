...
const nextConfig = {
  ...
  // Canonical redirect to enforce www.autogo.pt (avoids duplicate URLs)
  async redirects() {
    return [
      ...carRedirects,
      {
        source: '/:path*',
        has: [{ type: 'host', value: 'autogo.pt' }],
        destination: 'https://www.autogo.pt/:path*',
        permanent: true,
      },
    ];
  },

  // Remote image patterns (improves performance and prevents broken images)
  images: {
    remotePatterns: [
