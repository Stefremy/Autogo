...
export default function CarDetail() {
  ...
  const car = (cars as Car[]).find((c) => String(c.slug) === String(slug));
  ...
  const similarCars = (cars as Car[])
    .filter((c) => String(c.slug) !== String(car.slug))
  ...
  `,
                  ...
                  >
                    {similarCars.map((simCar, idx) => (
                      <div
                        ...
                        data-id={simCar.slug}
                      >
                        <a href={`/cars/${simCar.slug}`} className="block h-full">
      ...
      <Head>
        ...
        <meta
          property="og:url"
          content={
            car ? `https://autogo.pt/cars/${car.slug}` : "https://autogo.pt/cars/"
          }
        />
...
export async function getStaticPaths({ locales }) {
  ...
  for (const locale of locales) {
    for (const car of cars) {
      paths.push({ params: { slug: String(car.slug) }, locale });
    }
  }
